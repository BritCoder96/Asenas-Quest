...
def main():       
    ...
    arrow_surf = pygame.image.load("Arrow2.png").convert_alpha()
    level2_file = open("Asena's Quest/game/level_data/level_2.txt")
    ...
    arrow_timer = 0
    pygame.mixer.music.load("Okami - Battle of Ninetails.ogg")
    ...
    while (asena.hp > 0) and (level_complete == False):
        ...
        for row in range(len(level2)):
            for column in range(len(level2[row])):
                ...
                elif level2[row][column] == 'k':
                    if asena.has_key == False:
                        key.draw(screen)
                elif level2[row][column] == 'd':
                    door.draw(screen)
         #shoot arrows     
        arrow_timer += elapsed
